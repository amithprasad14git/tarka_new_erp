// Application API route — SARFAESI 13(2) paper publication covering sheet PDF download.

/**
 * GET /api/sarfaesi-case-status-update/covering-132-paper-publication-pdf/:id
 *
 * Same data load as covering-132-pdf; uses Paper Publication title/intro/note
 * and date from “13(2) Acknowledgements Received?” remarks.
...
export async function GET(req, { params }) {
  try {
    ...
    // --- Build Paper Publication PDF and return download ---
    const buffer = await buildSarfaesiCovering132PaperPublicationPdfBuffer({
      statusUpdate,
      nciRow,
      branchContext,
      signatoryName,
    ...
    const filename = safeSarfaesiCovering132PaperPublicationPdfFilename(refNo || id);
    ...
    return jsonApiErrorForAction(e, "downloadPdf", {
      logLabel: "sarfaesi covering-132 paper-publication pdf"
    });
  }
}
