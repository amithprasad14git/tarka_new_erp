...
/** True when any field on the module (or its child grids) points at this lookup module. */
...
export async function canAccessLovViaReferencingModule(user, lookupModuleKey) {
  ...
  // Grant LoV access when the user can view/create/edit any parent module that references this lookup.
  ...
  for (const parentKey of Object.keys(reports)) {
    if (parentKey === lookupModuleKey) continue;
    const r = reports[parentKey];
    if (!moduleConfigReferencesLookup(r, lookupModuleKey)) continue;
    const v = await hasModulePermission(user, parentKey, "view");
    if (v) return true;
  }
