...
function registerPreferredSansFonts(doc) {
  ...
  // Built-in PDFKit sans-serif fallback (similar to Arial/web sans).
...
export function buildNewCaseInwardBranchCopyPdf({
  ...
  const rboLabel = bankCode === "SBI" ? "RBO" : bankCode === "BOI" ? "AO / ZO" : "RO";
  ...
  return new Promise((resolve, reject) => {
    ...
    // Keep a clearly visible vertical gap after the date row.
    y += sectionGap * 2;
    ...
    if (fs.existsSync(footerPath)) {
      ...
      // Scale by width (not fit box) so footer spans page width like the legacy sample.
