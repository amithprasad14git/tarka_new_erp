/**
 * rbacMatrixModules — reusable utility for the ERP (server-side or shared).
 */
...
/**
 * @returns {{ key: string, label: string, group: string, isReport?: boolean }[]}
 */
export function getRbacMatrixModuleEntries() {
  ...
  const reportRows = getRbacMatrixReportEntries();
  return [...crud, ...reportRows]
    // Sort by menu group first, then label, for a stable matrix layout.
    .sort((a, b) => {
