...
/**
 * @typedef {{ ok: true, temperatureC: number, weatherCode: number, cityLabel: string } | { ok: false, message: string }} WeatherFetchResult
 */
...
export async function getWeatherWithFallback() {
  ...
  try {
    ...
    if (!res.ok || !body?.ok) {
      return { ok: false, message: apiUserMessage("weatherUnavailable") };
    }
