...
/** Stored / compared value when Final Invoice is checked. */
export const FINAL_INVOICE_YES = "Yes";
/** Stored / compared value when Final Invoice is not checked. */
export const FINAL_INVOICE_NO = "No";

const INVOICE_MODULE_KEYS = ["recovery_invoice", "sarfaesi_invoice", "vehicle_invoice"];

/** Parent fields that must be unchanged for a pure Final Invoice unlock edit. */
const UNLOCK_WATCHED_PARENT_FIELDS = [
  "caseNo",
  "billToUnit",
  "npaCurrentAc",
  "cancelledInvoice",
...
function unlockParentFieldUnchanged(oldRow, merged, fieldName) {
  const oldVal = rowValueForField(oldRow, fieldName);
  const newVal =
    merged != null && Object.prototype.hasOwnProperty.call(merged, fieldName)
      ? rowValueForField(merged, fieldName)
      : oldVal;
  return String(oldVal ?? "").trim() === String(newVal ?? "").trim();
}

/**
 * True when an existing invoice update only unlocks the case: Final Invoice Yes → No,
 * with date, case, billing fields, and charge total unchanged.
 *
 * @param {Record<string, unknown>|null|undefined} oldRow
 * @param {Record<string, unknown>|null|undefined} merged
 * @param {{ childTotal?: number }} [opts]
 */
export function isInvoiceFinalInvoiceUnlockUpdate(oldRow, merged, { childTotal } = {}) {
  if (!oldRow || !merged) return false;
  if (!Object.prototype.hasOwnProperty.call(merged, "finalInvoice")) return false;

  const oldFinal = normalizeFinalInvoiceFlag(rowValueForField(oldRow, "finalInvoice"));
  const newFinal = normalizeFinalInvoiceFlag(merged.finalInvoice);
  if (oldFinal !== FINAL_INVOICE_YES || newFinal !== FINAL_INVOICE_NO) return false;

  const oldDate = toYyyyMmDdForSqlDateField(rowValueForField(oldRow, "date"));
  const newDate = toYyyyMmDdForSqlDateField(
    Object.prototype.hasOwnProperty.call(merged, "date")
      ? merged.date
      : rowValueForField(oldRow, "date")
  );
  if (!oldDate || !newDate || oldDate !== newDate) return false;

  for (const field of UNLOCK_WATCHED_PARENT_FIELDS) {
    if (!unlockParentFieldUnchanged(oldRow, merged, field)) return false;
  }

  if (childTotal !== undefined && childTotal !== null) {
    const oldGt = Number(rowValueForField(oldRow, "grandTotal"));
    const roundedChild = Math.round(Number(childTotal) * 100) / 100;
    const roundedOld = Math.round(oldGt * 100) / 100;
    if (!Number.isFinite(roundedOld) || roundedOld !== roundedChild) return false;
  }

  return true;
}

/** Normalize Yes/No final-invoice flag for invoice modules. */
