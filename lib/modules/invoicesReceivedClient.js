...
/** Re-export amount/FK helpers so UI imports stay on the client module. */
...
/** True when the current screen is Invoices Received. */
export function isInvoicesReceivedModule(moduleKey) {
  return moduleKey === "invoices_received";
}

/** Invoices Received shows a linked invoice snapshot (not NCI case snapshot). */
...
/**
 * Build two-card snapshot detail (invoice fields + optional case details) for the modal.
 * @param {object} payload — API response with `data` invoice row
 * @param {string} invoiceModuleKey — recovery_invoice | sarfaesi_invoice | vehicle_invoice
 * @returns {Promise<{ cards: object[], recordId: unknown } | null>}
 */
...
/**
 * React hook: exclusive invoice FK pick, TDS/received amounts, and invoice snapshot modal.
 * @param {{ moduleKey: string, config: object, editingRow: object | null, formKey: string | number }} props
 */
...
/** Modal shell for Invoice Snapshot (loading / empty / detail states). */
