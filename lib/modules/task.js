...
function assertDateNotInPast(ymd, label) {
  ...
  if (ymd < today) {
    throwTaskValidation(`${label} cannot be in the past.`);
  }
}
...
/**
 * Insert a task activity-log child row (field change audit trail).
 * @param {import("mysql2/promise").PoolConnection} conn
 * @param {{ taskId: number, fieldName: string, fromValue: unknown, toValue: unknown, changedBy: unknown }} ctx
 */
...
/**
 * After create: seed activity log for the new task.
 * @param {import("mysql2/promise").PoolConnection} conn
...
/**
 * After update: log field changes on the task.
 * @param {import("mysql2/promise").PoolConnection} conn
...
/** Re-export child-table syncMode normalizer for task adapters. */
