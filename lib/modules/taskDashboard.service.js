...
/** Allowed task_master.status values. */
...
/**
 * SQL scope for task alerts: both assigned-to-me and assigned-by-me buckets.
 */
function appendTaskAlertsScope(user, whereParts, whereValues) {
  const uid = asPositiveInt(user?.id);
  if (!uid) {
    whereParts.push("1=0");
    return;
  }
  const assigneeCol = mysql.escapeId("assignee");
  const followUpCol = mysql.escapeId("followUpPerson");
  const createdByCol = mysql.escapeId("createdBy");
  whereParts.push(
    `(${assigneeCol} = ? OR ${followUpCol} = ? OR (${createdByCol} = ? AND ${assigneeCol} <> ?))`
  );
  whereValues.push(uid, uid, uid, uid);
}
...
const ALERT_ITEMS_LIMIT = 10;

/** Due/overdue task alerts for the top-bar bell. */
export async function loadTaskAlerts(user) {
  const assignedToMe = await queryTaskMetrics(user, "assigned_to_me");
  const assignedByMe = await queryTaskMetrics(user, "assigned_by_me");
  const overdueCount =
    (Number(assignedToMe.overdueTasks) || 0) + (Number(assignedByMe.overdueTasks) || 0);
  const dueTodayCount = (Number(assignedToMe.dueToday) || 0) + (Number(assignedByMe.dueToday) || 0);
  ...
  const whereParts = [];
  const whereValues = [];
  appendTaskAlertsScope(user, whereParts, whereValues);

  const statusCol = mysql.escapeId("status");
  const dueCol = mysql.escapeId("dueDate");
  const idCol = mysql.escapeId("id");
  const titleCol = mysql.escapeId("taskTitle");
  whereParts.push(`${statusCol} NOT IN ('Completed', 'Cancelled')`);
  whereParts.push(`${dueCol} IS NOT NULL`);
  whereParts.push(`DATE(${dueCol}) <= CURDATE()`);

  const whereSql = whereParts.length ? `WHERE ${whereParts.join(" AND ")}` : "";
  const [rows] = await pool.query(
    `SELECT ${idCol}, ${titleCol}, ${dueCol}, ${statusCol},
            ${mysql.escapeId("assignee")}, ${mysql.escapeId("createdBy")}, ${mysql.escapeId("createdDate")}
     FROM ${t} ${whereSql}
     ORDER BY ${dueCol} ASC, ${idCol} DESC
     LIMIT ?`,
    [...whereValues, ALERT_ITEMS_LIMIT]
  );

  const enriched = await enrichRowsWithUserLabels(rows || []);
  const items = enriched.map((r) => {
    const due = r.dueDate ? String(r.dueDate).slice(0, 10) : null;
    const created = r.createdDate ? String(r.createdDate).slice(0, 10) : null;
    return {
      id: r.id,
      taskTitle: r.taskTitle || "",
      dueDate: due,
      status: r.status,
      isOverdue: Boolean(due && due < todayYmd),
  ...
  return { overdueCount, dueTodayCount, alertCount, items };
}
...
/** List tasks for dashboard (My / Assigned / All bucket + optional status). */
...
/** Count tasks per status within a dashboard bucket. */
...
/** Load one task plus comments/activity for the dashboard detail panel. */
...
/** Create a task from the dashboard Create modal. */
...
/** Update status/fields on a task from the dashboard detail panel. */
...
/** Re-export task access / bucket helpers used by API routes. */
export {
