...
/** True when the current screen is SARFAESI Invoice. */
 ...
 */
export function getSarfaesiInvoiceDotTone(row) {
...
export function sarfaesiInvoiceDotLabel(tone) {
  if (tone === "cancelled") return "Cancelled invoice";
  if (tone === "received") return "Received invoice";
  return "Pending invoice";
}

/** Show post-save Print acknowledgement when editing an existing SARFAESI Invoice. */
...
// --- PDF print (SARFAESI Invoice) — wired in MasterModuleClient; see README.md#sarfaesi-invoice-pdf ---
...
/** Fetches GET /api/sarfaesi-invoice/pdf/:id and triggers browser download. See README.md#sarfaesi-invoice-pdf. */
