...
/** Re-export status list for dashboard UI. */
...
/** Build month calendar cells with due-count badges for the reminders widget. */
...
/** Dashboard summary tiles (counts by status) for the logged-in user. */
...
/** Due/overdue reminder alerts for the top-bar bell. */
export async function loadReminderAlerts(user) {
  const metrics = await queryReminderMetrics(user);
  const overdueCount = Number(metrics.overdueReminders) || 0;
  const dueTodayCount = Number(metrics.dueToday) || 0;
  ...
  whereValues.push("Pending");
  whereParts.push(`${dueCol} IS NOT NULL`);
  whereParts.push(`DATE(${dueCol}) <= CURDATE()`);

  const whereSql = whereParts.length ? `WHERE ${whereParts.join(" AND ")}` : "";
  const [rows] = await pool.query(
    `SELECT ${idCol}, ${mysql.escapeId("reminderTitle")}, ${dueCol}, ${statusCol},
            ${mysql.escapeId("createdBy")}, ${mysql.escapeId("createdDate")}
     FROM ${t} ${whereSql}
...
/** List reminders for dashboard list/modal (optional status / dueDate filter). */
...
/** Count reminders per status for the logged-in user. */
...
function reminderPermissionsForUser(user, row) {
  ...
  const isOwner = Number(row?.createdBy) === asPositiveInt(user?.id);

  if (isCompleted && !admin) {
    return {
...
/** Load one reminder plus activity for the dashboard detail panel. */
...
/** Create a reminder from the dashboard Create modal. */
...
/** Update status/fields on a reminder from the dashboard detail panel. */
...
/** Re-export reminder access helpers used by API routes. */
export {
  userCanViewReminder,
  userCanEditReminder,
