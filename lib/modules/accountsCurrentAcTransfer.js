...
export async function validateAccountsCurrentAcTransferBeforeWrite(conn, { parentData }) {
  void conn;
  assertAccountsCurrentAcTransferFromToDifferent(parentData);
}

export async function applyAccountsCurrentAcTransferBeforeWrite(conn, { oldRow, merged }) {
  const parentData = oldRow ? { ...oldRow, ...merged } : merged;
  await validateAccountsCurrentAcTransferBeforeWrite(conn, { parentData });
}
