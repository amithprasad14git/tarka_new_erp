...
/**
 * accountsCurrentAcTransfer — business rules when records are created or updated.
...
async function assertAccountsCurrentAcTransferDateNotInFrozenFy(conn, parentData, user) {
  // Unit operators cannot move balances between current accounts in a frozen FY.
...
/** From Current AC and To Current AC must be different accounts. */
export function assertAccountsCurrentAcTransferFromToDifferent(parentData) {
  // Prevents a no-op transfer that would still consume a voucher number.
...
export async function assignAccountsCurrentAcTransferVoucherNo(conn, recordId) {
  // --- Voucher ACC/TRF/<yearCode>/#### after create ---
...
/** Before save: from≠to, FY freeze for role 2. */
export async function validateAccountsCurrentAcTransferBeforeWrite(conn, { parentData, user }) {
  // --- FY freeze and from/to current account must differ ---
...
/** CRUD beforeWrite hook — runs validate on the merged parent row. */
