 ...
 *   NPA Acknowledgement (view-row select + edit toolbar only; no post-save print).
 *
...
/** Label on the Print NPA Acknowledgement toolbar button. */
export function getSarfaesiNpaAckPrintButtonText() {
  return "Print NPA Acknowledgement";
}
...
/** Same visibility rules as covering sheets. */
export function getSarfaesiNpaAckPrintTargetId(args) {
  return getSarfaesiCovering132PrintTargetId(args);
}
...
/**
 * Download NPA Acknowledgement PDF for a status-update record.
 * @param {number|string|null|undefined} recordId
 * @param {string} [refHint]
 */
export async function downloadSarfaesiNpaAckPdf(recordId, refHint) {
  if (recordId == null || String(recordId).trim() === "") return;
  const res = await fetch(
    `/api/sarfaesi-case-status-update/npa-acknowledgement-pdf/${recordId}`
  );
  const fallbackName = refHint
    ? `NPA_ACK_${String(refHint).replace(/\//g, "_")}.pdf`
    : "NPA_ACK.pdf";
  await downloadBlobResponse(res, apiUserMessage("downloadPdf"), fallbackName);
}
