 ...
 *
 * Documentation:
 * - Module guide: README.md#recovery-invoice-pdf
...
/** SBI vendor code — shown only when case bank (header) is SBI, not current-account bank. */
...
/** Charge % cell text like "@   5   %"; blank when 0/invalid. @param {*} raw @returns {string} */
...
/** Fixed body height for recovery dual grids (header + N data rows). @param {number} rowCount @returns {number} */
...
/** Centered “Recovery Invoice” badge band; returns y after band. @returns {number} */
...
/** Single full-width maroon title row above a charges/details grid. */
...
/** TOTAL row spanning first two cols + amount in third (recovery-details side). */
...
/**
 * Side-by-side RECOVERY DETAILS + RECOVERY CHARGES tables (equal row count, min 3).
 ...
 * Case No may be empty on Recovery — bank/branch/borrower/recovery rows then print blank by design.
 * @param {PDFKit.PDFDocument} doc
