/**
 * accountsExpenseVoucher — browser-only behaviour (forms, pickers, Print/download).
 * Server save rules: lib/modules/accountsExpenseVoucher.js
 */

// Module-specific file: Expense voucher client behaviour (mirror accounts_assets_investments).

import { useEffect, useMemo, useState } from "react";
import { handlePartyInFavourOfAutoFill } from "./accountsInFavourOfClient";

/** Keep in sync with ACCOUNTS_EXPENSE_VOUCHER_UNIT_RESTRICT_ROLE in accountsExpenseVoucher.js */
const UNIT_RESTRICT_ROLE = 2;

export const ACCOUNTS_EXPENSE_VOUCHER_POST_CREATE_ACK_UI = {
  field: "voucherNo",
...
export function isAccountsExpenseVoucherModule(moduleKey) {
  return moduleKey === "accounts_expense_voucher";
}
...
export function useAccountsExpenseVoucherClientModel({
  moduleKey,
  editingRow,
  formKey,
  sessionRole,
  ...
  useEffect(() => {
    // Unit operators: pre-fill unit and first NPA current account on a new expense voucher.
    if (!isAccountsExpenseVoucherModule(moduleKey)) {
      setAutoValues({});
  ...
  const entryFieldUiOverrides = useMemo(() => {
    if (!isAccountsExpenseVoucherModule(moduleKey)) return null;
    return {
      npaCurrentAc: {
        lookup: {
  ...
  function handleFieldValueChange(fieldName, value) {
    if (!isAccountsExpenseVoucherModule(moduleKey)) return false;
    if (
      handlePartyInFavourOfAutoFill(fieldName, "paidTo", value, setAutoValues, () =>
