 ...
 *
 * In plain terms, staff must:
 * - Pick a case that is already in “Returned” status
 * - Tick at least one return reason and fill in the reason text
 * - Use a date that is not in the future and not in a frozen financial year (role 2)
 *
 * PDF letter: lib/modules/returnCasePdf.js — README.md#return-case-letter-pdf
 * Browser helpers: lib/modules/returnCaseClient.js
 */
...
/**
 * Build CC To line from bank short code and RBO full name (Return Case letter).
 * bankCode is bank_master.bankCode (UI label "Short Code"); compared case-insensitively.
 */
export function buildReturnCaseCcToLine(bankCode, rboFullName) {
  const name = String(rboFullName ?? "").trim();
  if (!name) return "";
  ...
  let title;
  if (code === "CAN") {
    title = "The Assistant General Manager";
  } else if (code === "BOI") {
    title = "The Zonal Manager";
  } else {
...
/**
 * Resolve CC To text for a New Case Inward id via Branch → RBO → HO/ZO → Bank.
 * @returns {{ ccTo: string }}
 */
export async function resolveReturnCaseCcToByCaseId(conn, caseId) {
  ...
  const brTable = escapeSqlTableIdForModuleConfig(modules.branch_master);
  const rboTable = escapeSqlTableIdForModuleConfig(modules.rbo_master);
  const hzTable = escapeSqlTableIdForModuleConfig(modules.ho_zo_master);
  const bmTable = escapeSqlTableIdForModuleConfig(modules.bank_master);

  const [rows] = await conn.query(
    `
    SELECT bm.bankCode AS bankCode, rbo.fullName AS rboFullName
    FROM ${nciTable} nci
    INNER JOIN ${brTable} br ON br.id = nci.branch
    INNER JOIN ${rboTable} rbo ON rbo.id = br.rbo_ro
    INNER JOIN ${hzTable} hz ON hz.id = rbo.ho_zo
    INNER JOIN ${bmTable} bm ON bm.id = hz.bank
    WHERE nci.id = ?
    LIMIT 1
    `,
    [id]
  ...
  const ccTo = buildReturnCaseCcToLine(
    rowValueForField(rows[0], "bankCode"),
    rowValueForField(rows[0], "rboFullName")
  );
  return { ccTo };
}

/**
 * Active return reasons for new Return Case child grid preload.
 * Authorized callers should gate on return_case permission, not case_return_reasons.
 * @returns {{ data: { returnReason: string, sequence: number|null }[] }}
 */
export async function loadActiveReturnReasonsForPreload(conn) {
  const table = escapeSqlTableIdForModuleConfig(modules.case_return_reasons);
  const [rows] = await conn.query(
    `SELECT returnReason, \`sequence\` FROM ${table} WHERE active = ? ORDER BY \`sequence\` ASC, id ASC`,
    ["Yes"]
  );
  const data = (rows || [])
    .map((row) => ({
      returnReason: String(rowValueForField(row, "returnReason") ?? "").trim(),
      sequence: rowValueForField(row, "sequence")
    }))
    .filter((row) => row.returnReason);
  return { data };
}
...
/**
 * Domain rules before Return Case create/update (Returned status, reasons, FY freeze).
 * @param {import("mysql2/promise").PoolConnection} conn
 * @param {object} ctx
 */
export async function validateReturnCaseBeforeWrite(conn, {
  parentData,
  childTableRows,
  parentRecordId = null,
  user
}) {
  // --- Date rules, FY freeze, case must be Returned, child reasons, one return per case ---
  ...
  if (shouldEnforceFreezeTransactionsForUser(user)) {
    await assertDateNotInFrozenFinancialYear(conn, parentData?.date, {
      onBlocked: () => throwReturnCaseValidation(FREEZE_TRANSACTIONS_LOCKED_MESSAGE)
    });
  }
...
/**
 * CRUD beforeWrite entry for Return Case.
 * @param {import("mysql2/promise").PoolConnection} conn
 * @param {object} ctx
 */
export async function applyReturnCaseBeforeWrite(conn, {
  oldRow,
  merged,
  childTableRows,
  parentRecordId = null,
...
/**
 * Stamp refNo as RC/&lt;yearCode&gt;/&lt;4-digit serial&gt; after INSERT.
 * @param {import("mysql2/promise").PoolConnection} conn
 * @param {number} recordId
 */
export async function assignReturnCaseRefNo(conn, recordId) {
  // --- Reference RETURN/<yearCode>/##### after INSERT ---
  const rcTable = escapeSqlTableIdForModuleConfig(modules.return_case);
