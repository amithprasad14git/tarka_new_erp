/**
 * =============================================================================
 * TRANSFER CASE (CLIENT) — Unit transfer form behaviour
 * =============================================================================
 * When a case moves between units, the form must lock date to today, fill From
 * Unit from the case, filter To Unit / Assignee LoVs, and clear assignee when
 * destination unit changes. Keeps that UI logic out of generic components.
 * Server save rules: lib/modules/transferCase.js
 * =============================================================================
 */
...
/** True when the current screen is Transfer Case. */
export function isTransferCaseModule(moduleKey) {
  return moduleKey === "transfer_case";
}

/** Transfer Case participates in the shared case-snapshot panel. */
...
/**
 * React hook: Transfer Case auto-values, LoV filters, and Case No → From Unit fill.
...
export function useTransferCaseClientModel({ moduleKey, editingRow, formKey, sessionRole, sessionUnit }) {
  ...
  function handleFieldValueChange(fieldName, value) {
    ...
    if (fieldName === "toUnit") {
      // Changing destination unit clears assignee — user must pick someone in that unit.
      setAutoValues((prev) => ({
        ...prev,
        toUnit: value,
        assignee: "",
    ...
    // New case selection resets units until NCI returns current owner as From Unit.
    setAutoValues((prev) => ({
      ...prev,
      toUnit: "",
      assignee: "",
