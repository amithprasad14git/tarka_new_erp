...
/**
 * newCaseInwardCaseDetailsPdf — builds the downloadable PDF for this module.
...
function drawStatusMark(doc, caseStatus, x, y, w, h) {
  ...
  // Same visual language as grid status dots: X = returned, tick = final, dots = ongoing.
...
/**
 * Build Case Details PDF (summary + amount recovered) for one NCI case.
 * Called from the case-details-pdf API route.
 */
export function buildNewCaseInwardCaseDetailsPdf({
  ...
  // Sum child "amount recovered" lines for the summary block on the PDF.
  ...
  return new Promise((resolve, reject) => {
    ...
    // --- Case header rows and amount-recovered summary table ---
    let y = top + 125;
    const labelW = 155;
...
/** Download filename for Case Details PDF from case no. */
