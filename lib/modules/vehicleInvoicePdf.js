...
// --- Font sizes (points) — aligned with recoveryInvoicePdf.js (May 2026) ---
const FS_BODY = 9;
const FS_SECTION = 11;
const FS_TABLE_HEAD = 8;
const FS_COPY = 8;
/** RCM shrinks from FS_BODY down to this minimum to fit the account block height. */
const FS_RCM_MIN = 5.5;
const BOX_ROW_H = 17;
...
function fitRcmFontSize(doc, fonts, w, h) {
  ...
  for (let fs = FS_BODY; fs >= FS_RCM_MIN; fs -= 0.5) {
    doc.font(fonts.regular).fontSize(fs);
    const textH = doc.heightOfString(RCM_NOTE, {
      width: innerW,
