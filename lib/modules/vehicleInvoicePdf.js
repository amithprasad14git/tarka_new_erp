/**
 * Vehicle Invoice PDF generator (layout aligned with SARFAESI Invoice — May 2026).
 *
 * What this file does:
 ...
 *
 * Documentation:
 * - Module guide: docs/vehicle-invoice-pdf.md
...
const FS_BODY = 10;
const FS_SECTION = 12;
const FS_TABLE_HEAD = 9;
const FS_COPY = 9;
const FS_RCM_MIN = 4.5;
const FS_RCM_MAX = 7;
const BOX_ROW_H = 20;
const LINE_GAP = -1;
...
const FS_ACCOUNT = FS_BODY - 1;
const ACCOUNT_PAD_L = BOX_PAD_L - 1;
const ACCOUNT_PAD_T = BOX_PAD_T - 1;
const ACCOUNT_ROW_H = 18;
...
function drawBranchLvText(doc, fonts, boxX, y, boxW, rowH, colonX, valueX, label, value, valueColor, textStyle = {}) {
  ...
  doc.text(pdfSafeLine(label), boxX + padL, y + padT, { width: labelW, lineBreak: false });
  doc.text(":", colonX, y + padT, { lineBreak: false });
...
function drawBranchStyleBlock(doc, fonts, boxX, y, boxW, pairs, colonPreset, textStyle = {}) {
  ...
  for (let i = 0; i < pairs.length; i++) {
    const ry = y + i * rowH;
    drawBranchRowBorders(doc, boxX, ry, boxW, rowH, i === 0, i === pairs.length - 1);
...
function drawInCell(doc, fonts, text, x, y, w, h, opts = {}) {
  ...
  if (safe) {
    ...
    const lineGap = opts.lineGap ?? LINE_GAP;
...
function drawFlowText(doc, fonts, text, x, y, width, opts = {}) {
  ...
  const safe = pdfSafeLine(text);
  doc.font(face).fontSize(fs).fillColor(opts.color ?? "#000000");
  const textH = doc.heightOfString(safe, { width, lineGap: LINE_GAP });
  doc.text(safe, x, y, { width, align: opts.align || "left", lineGap: LINE_GAP, lineBreak: false });
  restoreDocCursor(doc, saved);
  return y + textH;
}
...
function drawRcmBlock(doc, fonts, x, y, w, h) {
  const innerW = Math.max(1, w - CELL_PAD_W * 2);
  const innerH = Math.max(1, h - CELL_PAD_H * 2);
  let chosenFs = FS_RCM_MIN;
  for (let fs = FS_RCM_MAX; fs >= FS_RCM_MIN; fs -= 0.5) {
    doc.font(fonts.regular).fontSize(fs);
    const textH = doc.heightOfString(RCM_NOTE, { width: innerW, lineGap: LINE_GAP });
    if (textH <= innerH) {
      chosenFs = fs;
      break;
    }
  }
  drawInCellMultiline(doc, fonts, RCM_NOTE, x, y, w, h, {
    bold: false,
    fontSize: chosenFs,
    align: "justify",
    lineGap: LINE_GAP
...
function drawAccountWithRcm(doc, fonts, leftX, y, contentW, ca, bankCode) {
  ...
  const blockHeight = pairs.length * ACCOUNT_ROW_H;
  drawBranchStyleBlock(doc, fonts, leftX, blockTop, accountW, pairs, "accountLabels", accountTextStyle);
...
function drawTextLine(doc, fonts, text, x, y, width, opts = {}) {
  ...
  const safe = pdfSafeLine(text);
  doc.font(face).fontSize(fs).fillColor(opts.color ?? "#000000");
  const textH = doc.heightOfString(safe, { width: innerW, lineGap: LINE_GAP });
  const ty = y + CELL_PAD_H + Math.max(0, (innerH - textH) / 2);
  doc.text(safe, x + CELL_PAD_W, ty, {
    width: innerW,
    height: innerH,
