...
/**
 * publicNoticePdf — builds the downloadable PDF for this module.
...
/**
 * Build the Public Notice PDF buffer (bank header, defaulters, loan summary).
 * Called from the public-notice PDF API route.
 */
export function buildPublicNoticePdfBuffer(input) {
  // Single-page statutory notice: bank header, up to three defaulters, loan summary, closing text.
  ...
  return new Promise((resolve, reject) => {
    ...
    // --- Header: bank logo, RBO name, branch/date, “PUBLIC NOTICE” title ---
    ...
    // --- Body: RBI default notice text, then up to three defaulter columns ---
    const bodyIntro =
    ...
    while (introFs >= 8) {
      ...
      const yAddrStart = yAfterIntro + boxH + gapAfterBoxes + typeRowH + nameRowH;
      const minAddr = mm(8);
    ...
    // Photo placeholder boxes (one per defaulter column).
    ...
    // --- Loan account, liability amount, and statutory closing paragraph ---
...
/** Download filename, e.g. PUBLIC_NOTICE_CASE-001.pdf */
