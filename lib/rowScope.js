 ...
 * or **unit** (only rows whose *creator’s* business unit matches yours — or the module’s
 * `rowScopeUnitField` when configured, e.g. New Case Inward `unit`).
 *
...
/** Extra columns that count as "own" besides createdBy (e.g. task assignee). */
function ownScopeAlsoMatchFields(moduleConfig) {
  const raw = moduleConfig?.rowScopeOwnAlsoMatchFields;
  if (!Array.isArray(raw)) return [];
  const fn = fieldNameSet(moduleConfig);
  return raw.map((n) => String(n || "").trim()).filter((n) => n && fn.has(n));
}

/** When set on module config, unit scope matches this row field to users.unit (not creator). */
function resolveUnitScopeField(moduleConfig) {
  const field = String(moduleConfig?.rowScopeUnitField ?? "").trim();
  if (!field) return null;
  return fieldNameSet(moduleConfig).has(field) ? field : null;
}

function rowMatchesUnitScope(moduleConfig, user, row) {
  const uid = user?.unit != null && user.unit !== "" ? Number(user.unit) : null;
  if (!Number.isFinite(uid)) return false;
  const unitField = resolveUnitScopeField(moduleConfig);
  if (unitField) {
    const rowUnit = row[unitField];
    if (rowUnit == null || rowUnit === "") return false;
    return Number(rowUnit) === uid;
  }
  return null;
}

function appendUnitScopeSql(moduleConfig, user, whereParts, whereValues) {
  const uid = user?.unit != null && user.unit !== "" ? Number(user.unit) : null;
  ...
  const unitField = resolveUnitScopeField(moduleConfig);
  if (unitField) {
    whereParts.push(`${mysql.escapeId(unitField)} = ?`);
...
function appendOwnScopeSql(moduleConfig, user, whereParts, whereValues) {
  const cols = getAuditColumnNames(moduleConfig);
  const createdBy = cols.createdBy;
  const fn = fieldNameSet(moduleConfig);
  const hasCreatedBy = fn.has(createdBy);
  const extras = ownScopeAlsoMatchFields(moduleConfig);

  if (moduleConfig.table === "users" && hasCreatedBy) {
  ...
  const vals = [];
  if (hasCreatedBy) {
    clauses.push(`${mysql.escapeId(createdBy)} = ?`);
    vals.push(user.id);
  }
  for (const field of extras) {
    clauses.push(`${mysql.escapeId(field)} = ?`);
    vals.push(user.id);
  }
  if (!clauses.length) return;
  whereParts.push(clauses.length === 1 ? clauses[0] : `(${clauses.join(" OR ")})`);
  whereValues.push(...vals);
}

function rowMatchesOwnScope(moduleConfig, user, row) {
  const cols = getAuditColumnNames(moduleConfig);
  const createdBy = cols.createdBy;
  const fn = fieldNameSet(moduleConfig);
  const hasCreatedBy = fn.has(createdBy);
  ...
  if (hasCreatedBy && Number(row[createdBy]) === Number(user.id)) return true;
  for (const field of extras) {
    if (Number(row[field]) === Number(user.id)) return true;
  }
  return !hasCreatedBy && extras.length === 0;
}
...
export function appendRowScopeFilter(moduleConfig, user, scope, whereParts, whereValues) {
  ...
  if (sc === "own") {
    if (!hasCreatedBy && !ownScopeAlsoMatchFields(moduleConfig).length) return;
    appendOwnScopeSql(moduleConfig, user, whereParts, whereValues);
    return;
  }

  if (sc === "unit") {
    appendUnitScopeSql(moduleConfig, user, whereParts, whereValues);
  }
}
...
export async function rowMatchesScope(moduleConfig, user, scope, row) {
  ...
  if (sc === "own") {
    if (!hasCreatedBy && !ownScopeAlsoMatchFields(moduleConfig).length) return true;
    return rowMatchesOwnScope(moduleConfig, user, row);
  }

  if (sc === "unit") {
    const byRowUnit = rowMatchesUnitScope(moduleConfig, user, row);
    if (byRowUnit !== null) return byRowUnit;
    const uid = user?.unit != null && user.unit !== "" ? Number(user.unit) : null;
    if (!Number.isFinite(uid)) return false;
...
export async function annotateRowsModifyAccess(moduleKey, moduleConfig, user, rows, flags) {
  ...
  const uid = user?.unit != null && user.unit !== "" ? Number(user.unit) : null;
  const rowUnitField = resolveUnitScopeField(moduleConfig);
  ...
  if (
    ...
    // Batch-fetch creator units once instead of one query per grid row.
    const ids = new Set();
    for (const row of rows) {
      const id = row[createdBy];
  ...
  function matches(scopeRaw, row) {
    ...
    if (sc === "unit") {
      if (rowUnitField) {
        if (!Number.isFinite(uid)) return false;
        const rowUnit = row[rowUnitField];
        if (rowUnit == null || rowUnit === "") return false;
        return Number(rowUnit) === uid;
      }
      if (!hasCreatedBy) return false;
