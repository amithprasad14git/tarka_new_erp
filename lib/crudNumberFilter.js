/**
 * crudNumberFilter — reusable utility for the ERP (server-side or shared).
 */
...
export function appendNumberColumnFilter(fieldName, field, rawValue, whereParts, whereValues) {
  ...
  if (shouldUseExactNumberColumnFilter(field)) {
    ...
    if (n == null) return;
    // Flags and IDs must match exactly, not partial digits.
    whereParts.push(`${col} = ?`);
  ...
  // Amount columns: partial digit match (100 matches 1000) via CAST … LIKE.
