...
function validateChildTableRow(ct, row, rowIndex) {
  ...
  // Check each column on this child line for required/type rules.
 ...
 * @param {unknown} v
 */
/** Coerces one child cell to the DB type (null for empty optional fields). */
...
export async function syncChildTablesInTransaction(conn, moduleConfig, parentId, childTableRows) {
  ...
  for (const ct of children) {
    ...
    // Replace strategy: wipe old lines, insert the payload rows fresh.
