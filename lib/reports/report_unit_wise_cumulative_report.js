 ...
 * Data Type: Month Wise (banded month × unit) or Unit Wise (flat one row per unit).
 * Excel: lib/reports/custom/report_unit_wise_cumulative_report/buildCustomWorkbook.js
 * Config: config/reports.js → report_unit_wise_cumulative_report. See README.md#reports-frozen-framework.
 */
...
import {
  UNIT_WISE_CUMULATIVE_DATA_TYPE_UNIT_WISE
} from "./unitWiseCumulativeDataTypes.js";

export { buildCustomWorkbook } from "./custom/report_unit_wise_cumulative_report/buildCustomWorkbook.js";
export {
  UNIT_WISE_CUMULATIVE_DATA_TYPE_MONTH_WISE,
  UNIT_WISE_CUMULATIVE_DATA_TYPE_UNIT_WISE
} from "./unitWiseCumulativeDataTypes.js";

/** Safe quoted table names for unit-wise cumulative joins. */
...
/**
 * Month Wise: aggregates settled cases with cash recovered > 0 by settlement month × unit.
 * @returns {string}
 */
...
/**
 * Unit Wise: one aggregated row per unit (settled in FY with cash recovered > 0).
 * @returns {string}
 */
...
/**
 * FY window on caseStatusUpdatedDate + settled status + optional bank hierarchy filters.
...
/** Maps Unit Wise SQL rows to custom layout row shape. */
...
/**
 * Runs Unit Wise Cumulative Report (Month Wise or Unit Wise) for the selected FY.
 * @param {object} user
 * @param {Record<string, unknown>} filters
