...
/**
 * Splits ledger amount into receipt vs payment columns by transaction type.
...
/** Safe quoted table names for loan account ledger joins. */
 ...
 */
export function buildLoanAccountLedgerWhereSql(filters) {
  const parts = [];
  const values = [];
...
/** Detail SELECT for loan account ledger vouchers. */
...
/**
 * Runs Loan Account Ledger cumulative as of the selected date.
 * @param {object} user
 * @param {Record<string, unknown>} filters
 * @param {{ limit?: number }} ctx
 */
export async function runReport(user, filters, ctx = {}) {
  const { whereSql, values } = buildLoanAccountLedgerWhereSql(filters);
