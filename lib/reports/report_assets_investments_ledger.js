...
/** Safe quoted table names for assets & investments ledger joins. */
 ...
 */
export function buildAssetsInvestmentsLedgerWhereSql(filters) {
  const parts = [];
  const values = [];
...
/** Detail SELECT for assets & investments vouchers. */
...
/**
 * Runs Assets & Investments Ledger for the selected month range.
 * @param {object} user
 * @param {Record<string, unknown>} filters
 * @param {{ limit?: number }} ctx
 */
export async function runReport(user, filters, ctx = {}) {
  const { whereSql, values } = buildAssetsInvestmentsLedgerWhereSql(filters);
