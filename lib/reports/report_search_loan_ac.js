// Report — Search Loan AC. All SQL and filter WHERE logic for this report only.

/**
 * Case search by loan AC / name / case no; Data Type filter (All/Ongoing/Settled/Returned).
 * Config: report_search_loan_ac.
 */
...
import { buildReturnedCaseStatusWhereSql } from "./report_returned_cases.js";
import { buildSettledCaseStatusWhereSql } from "./report_settled_cases.js";

/** Safe quoted table names for search-loan-AC NCI joins. */
...
/**
 * Normalizes Data Type filter; default and empty both mean All (no case-status filter).
 * @param {unknown} dataType
 * @returns {"All" | "Ongoing" | "Settled" | "Returned"}
 */
export function normalizeSearchLoanAcDataType(dataType) {
  const norm = String(dataType ?? "All").trim();
  if (!norm || norm.toLowerCase() === "all") return "All";
  if (norm === "Ongoing" || norm === "Settled" || norm === "Returned") return norm;
  return "All";
}

/**
 * Case-status filter from Data Type select (All = no extra filter — all statuses).
 * @param {unknown} dataType
 * @returns {{ sql: string, values: string[] } | null} null when All (no status predicate)
 */
export function buildDataTypeStatusWhereSql(dataType) {
  const norm = normalizeSearchLoanAcDataType(dataType);
  if (norm === "All") return null;
  if (norm === "Ongoing") {
    return buildOpenCaseStatusWhereSql();
  }
  if (norm === "Settled") {
    return buildSettledCaseStatusWhereSql();
  }
  return buildReturnedCaseStatusWhereSql();
}

/**
 * Optional partial-match text search on loan AC, borrower name, case no.
 * @param {Record<string, unknown>} filters
 * @returns {{ parts: string[], values: string[] }}
 */
export function buildTextSearchWhereParts(filters) {
  const parts = [];
  const values = [];

  const loanAc = String(filters.searchLoanAc ?? "").trim();
  if (loanAc) {
    parts.push("nci.loanAccountNo LIKE ? ESCAPE '\\\\'");
    values.push(`%${escapeSqlLikePattern(loanAc)}%`);
  }

  const name = String(filters.searchName ?? "").trim();
  if (name) {
    parts.push("nci.borrower LIKE ? ESCAPE '\\\\'");
    values.push(`%${escapeSqlLikePattern(name)}%`);
  }

  const caseNo = String(filters.searchCaseNo ?? "").trim();
  if (caseNo) {
    parts.push("nci.caseNo LIKE ? ESCAPE '\\\\'");
    values.push(`%${escapeSqlLikePattern(caseNo)}%`);
  }

  return { parts, values };
}

/** SELECT for loan-AC search results with status, recovered amount, and bank hierarchy. */
function buildSelectSql() {
  ...
  SELECT
    ...
    (SELECT COALESCE(SUM(ar.recoveredAmount), 0)
     FROM ${t.ar} ar
     WHERE ar.caseInwardId = nci.id) AS amountRecovered
  FROM ${t.nci} nci
...
function buildWhere(filters) {
  ...
  const dataTypeFilter = buildDataTypeStatusWhereSql(filters.dataType);
  if (dataTypeFilter) {
    parts.push(dataTypeFilter.sql);
    values.push(...dataTypeFilter.values);
  }

  const textSearch = buildTextSearchWhereParts(filters);
  parts.push(...textSearch.parts);
  values.push(...textSearch.values);
...
/**
 * Runs Search Loan AC with optional text search and Data Type status filter.
 * @param {object} user
 * @param {Record<string, unknown>} filters
