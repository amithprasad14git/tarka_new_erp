...
/** Excel character widths for primary band columns (1-based index). */
const PRIMARY_COLUMN_WIDTHS = {
  1: 6,
  2: 12,
  3: 28,
  4: 22,
...
const PARTICULAR_COL_WIDTH = 12;
const AMOUNT_RECOVERED_COL_WIDTH = 14;
const REMARKS_COL_WIDTH = 24;
...
export async function buildCustomWorkbook(reportConfig, payload) {
  ...
  for (let i = 0; i < cases.length; i++) {
    if (i > 0) rowIdx += 1;
    rowIdx = writeCaseBlock(ws, cases[i], particulars, rowIdx);
  }

  const subColumns = buildSubColumns(particulars);
  const remarksCol = 1 + subColumns.length;
  const amountRecoveredCol = remarksCol - 1;

  ws.getColumn(1).width = PRIMARY_COLUMN_WIDTHS[1];
  for (let c = 2; c <= colCount; c++) {
    if (c === remarksCol) {
      ws.getColumn(c).width = REMARKS_COL_WIDTH;
    } else if (c === amountRecoveredCol) {
      ws.getColumn(c).width = AMOUNT_RECOVERED_COL_WIDTH;
    } else if (PRIMARY_COLUMN_WIDTHS[c] != null) {
      ws.getColumn(c).width = PRIMARY_COLUMN_WIDTHS[c];
    } else {
      ws.getColumn(c).width = PARTICULAR_COL_WIDTH;
    }
  }
