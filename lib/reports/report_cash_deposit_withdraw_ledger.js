...
/** MySQL DATE_FORMAT for month section labels in annual cash D/W summary. */
export const MONTH_LABEL_FORMAT_CASH_DEPOSIT_WITHDRAW = "%M-%Y";

/** Safe quoted table names for cash deposit/withdraw joins. */
 ...
 */
export function buildCashDepositWithdrawLedgerWhereSql(filters, dateRange = null) {
  const parts = [];
  const values = [];
 ...
 * @returns {string}
 */
export function buildCashDepositWithdrawFromJoinSql() {
  const t = sqlTableIds();
  return `
  FROM ${t.acdw} acdw
...
/** Detail-row SELECT for the monthly cash deposit/withdraw ledger. */
function buildSelectSql() {
  ...
  ${buildCashDepositWithdrawFromJoinSql()}
`;
}

/**
 * Summary SELECT (NPA AC + month + amount) — reused by annual cash D/W ledger.
 * @returns {string}
 */
export function buildCashDepositWithdrawSummarySelectSql() {
  return `
  SELECT
    cam.branch AS npaCurrentAcLabel,
    DATE_FORMAT(acdw.date, '${MONTH_LABEL_FORMAT_CASH_DEPOSIT_WITHDRAW}') AS monthLabel,
    DATE_FORMAT(acdw.date, '%Y-%m') AS monthKey,
    acdw.amount AS amount
  ${buildCashDepositWithdrawFromJoinSql()}
`;
}
...
/**
 * Runs Cash Deposit & Withdraw Ledger for the selected month and transaction type.
 * @param {object} user
 * @param {Record<string, unknown>} filters
 * @param {{ limit?: number }} ctx
 */
export async function runReport(user, filters, ctx = {}) {
  const { whereSql, values } = buildCashDepositWithdrawLedgerWhereSql(filters);
