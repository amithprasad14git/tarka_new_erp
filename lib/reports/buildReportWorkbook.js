...
// Frozen layout v1 — fonts, logo size, borders from reportExportTheme.excel. See README.md#reports-frozen-framework.
...
import { formatReportCellValue } from "../formatReportCellValue";
...
import {
  ...
  estimateRowHeightFromCells,
  resolveExcelColumnWidth
} from "./excelWrapLayout";
...
/** Same formatting as HTML (dates, INR amounts, whole-number counts). */
function formatCellValue(col, value) {
  if (value == null || value === "") return "";
  if (col.type === "date" || col.type === "inr" || col.type === "number") {
    return formatReportCellValue(col, value);
  }
  return value;
}
...
function excelAlignmentForColumn(col, horizontal) {
  ...
  const nowrap = col.type === "inr" || col.type === "number" || col.type === "date";
...
export async function buildReportWorkbook(reportConfig, payload) {
  ...
  if (payload.filterSummary) {
    ...
    ws.mergeCells(rowIdx, 1, rowIdx, colCount);
    // Approximate wrap using full table width so long filter lines open fitted.
    const filterWidth = Math.min(
      columns.reduce((sum, col) => sum + (Number(col.widthExcel) || 12), 0) || 60,
      80
    );
    r.height = estimateRowHeightFromCells(
      [{ text: payload.filterSummary, columnWidth: filterWidth, wrap: true }],
      excelTheme.filterFontSize || 10
    );
    rowIdx++;
  }
  rowIdx++;

  // Collect sample cell text for nowrap width estimates (amounts/dates).
  const sampleByCol = columns.map(() => []);
  const pushSamples = (dataRows) => {
    for (const dataRow of dataRows || []) {
      columns.forEach((col, i) => {
        if (sampleByCol[i].length >= 40) return;
        sampleByCol[i].push(formatCellValue(col, dataRow[col.key]));
      });
    }
  };
  if ((payload.groupedSections || []).length > 0) {
    for (const section of payload.groupedSections) {
      pushSamples(section.rows);
      for (const monthGroup of section.monthGroups || []) {
        pushSamples(monthGroup.rows);
      }
    }
  } else {
    pushSamples(payload.rows);
  }

  const columnWidths = columns.map((col, i) => {
    const wraps = excelColumnWrapsText(col);
    const samples = [col.label, ...(sampleByCol[i] || [])];
    return resolveExcelColumnWidth(col.widthExcel, samples, { wrapsText: wraps });
  });
  ...
  headerRow.height = estimateRowHeightFromCells(
    columns.map((col, i) => ({
      text: col.label,
  ...
  function writeDataRow(dataRow, ri, isLastDataRow) {
    ...
    columns.forEach((col, ci) => {
      ...
      const display = formatCellValue(col, dataRow[col.key]);
      cell.value = display;
      ...
      heightCells.push({
        text: display,
        columnWidth: columnWidths[ci],
        wrap: excelColumnWrapsText(col)
      });
    });
    applyDataRowBorders(r, colCount, { isLastDataRow });
    r.height = estimateRowHeightFromCells(heightCells, excelTheme.fontSize);
  }
  ...
  function writeSumRow(sumValues, labelText, fillArgb, overrideLabelColKey = null) {
    ...
    columns.forEach((col, ci) => {
      ...
      if (col.sum && sumValues?.[col.key] != null) {
        display = formatCellValue(col, sumValues[col.key]);
        cell.value = display;
        // Counts use column align; amounts stay right-aligned.
        cell.alignment = excelAlignmentForColumn(
          col,
          col.type === "number" ? col.align || "center" : "right"
        );
