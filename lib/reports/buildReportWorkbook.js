...
/** All report table cells use wrap text in Excel. */
export function excelCellAlignment(horizontal = "left", vertical = "middle") {
...
export async function buildReportWorkbook(reportConfig, payload) {
  ...
  function writeDataRow(dataRow, ri, isLastDataRow) {
    ...
    columns.forEach((col, ci) => {
      ...
      if (col.align === "right") cell.alignment = excelCellAlignment("right");
      else if (col.align === "center") cell.alignment = excelCellAlignment("center");
      else cell.alignment = excelCellAlignment("left");
    });
    applyDataRowBorders(r, colCount, { isLastDataRow });
  }

  function writeSumRow(sumValues, labelText, fillArgb) {
    const r = ws.getRow(rowIdx++);
    columns.forEach((col, ci) => {
      const cell = r.getCell(ci + 1);
      ...
      if (col.sum && sumValues?.[col.key] != null) {
        ...
        cell.alignment = excelCellAlignment("right");
      } else if (ci === (labelColIndex >= 0 ? labelColIndex : 0)) {
        cell.value = labelText;
        cell.alignment = excelCellAlignment(col.align === "right" ? "right" : "left");
      }
    });
  ...
  if (groupedSections.length > 0) {
    const totalDetailRows = groupedSections.reduce((n, s) => n + (s.rows?.length || 0), 0);
