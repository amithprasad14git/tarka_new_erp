...
import pool from "../db";
import { ACCOUNTS_EXPENSE_VOUCHER_UNIT_RESTRICT_ROLE } from "../modules/accountsExpenseVoucher";
 ...
 */
export function buildExpenseLedgerWhereSql(filters, user = null) {
  const parts = [];
  const values = [];
  ...
  if (Number.isFinite(role) && role === ACCOUNTS_EXPENSE_VOUCHER_UNIT_RESTRICT_ROLE) {
...
export async function runReport(user, filters, ctx = {}) {
  ...
  const { whereSql, values } = buildExpenseLedgerWhereSql(filters, user);
