...
/** MySQL DATE_FORMAT for month section labels (e.g. "April-2025"). */
export const MONTH_LABEL_FORMAT_EXPENSE_LEDGER = "%M-%Y";

/** Flat chronological ledger (default data type). */
export const EXPENSE_LEDGER_DATA_TYPE_GENERAL = "General";
/** Group detail rows by payment mode (Card/Cheque/Cash/UPI). */
export const EXPENSE_LEDGER_DATA_TYPE_PAYMENT_MODE = "Payment Mode Wise";
/** Group detail rows by expense category lookup. */
export const EXPENSE_LEDGER_DATA_TYPE_EXPENSE_CATEGORY = "Expense Category Wise";

/** Safe quoted table names for expense voucher joins. */
 ...
 */
export function buildExpenseLedgerWhereSql(filters, dateRange = null) {
  const parts = [];
  const values = [];

  const from = dateRange?.from || monthStartYmd(String(filters.month || "").trim());
  const to = dateRange?.to || monthEndYmd(String(filters.month || "").trim());
...
/** ORDER BY for General vs payment-mode / category-wise grouping. */
...
/**
 * FROM/JOIN fragment shared by detail and annual summary selects.
 * @returns {string}
 */
export function buildExpenseLedgerFromJoinSql() {
  const t = sqlTableIds();
  return `
  FROM ${t.aev} aev
...
/** Detail-row SELECT for the monthly expense ledger. */
function buildSelectSql() {
  ...
  ${buildExpenseLedgerFromJoinSql()}
`;
}

/**
 * Summary SELECT splitting amount by payment mode — reused by annual expense ledger.
 * @returns {string}
 */
export function buildExpenseLedgerSummarySelectSql() {
  return `
  SELECT
    cam.branch AS npaCurrentAcLabel,
    DATE_FORMAT(aev.date, '${MONTH_LABEL_FORMAT_EXPENSE_LEDGER}') AS monthLabel,
    DATE_FORMAT(aev.date, '%Y-%m') AS monthKey,
    lvm.lookupValue AS expenseCategoryLabel,
    CASE WHEN aev.paymentMode = 'Card' THEN aev.amount ELSE 0 END AS byCard,
    CASE WHEN aev.paymentMode = 'Cheque' THEN aev.amount ELSE 0 END AS byCheque,
    CASE WHEN aev.paymentMode = 'Cash' THEN aev.amount ELSE 0 END AS byCash,
    CASE WHEN aev.paymentMode = 'UPI' THEN aev.amount ELSE 0 END AS byUpi
  ${buildExpenseLedgerFromJoinSql()}
`;
}

/** Maps a raw SQL row to the expense ledger detail output shape. */
...
/**
 * Runs Expense Ledger for the selected month (General or grouped data types).
 * @param {object} user
 * @param {Record<string, unknown>} filters
