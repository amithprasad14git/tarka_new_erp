 ...
 * - This validator is common for all modules.
 * - It checks shape/type/required basics only.
 * - Module-specific business rules must stay in `lib/modules/<module>.js`.
 *
...
function isEmptyValue(value, field) {
  ...
  const t = field.type;
  if (t === "lookup") {
    // Note: non-numeric lookup values are treated as "empty" at this stage.
    // - required lookup => "is required"
    // - optional lookup => skip format validation
    if (value === "") return true;
    return !Number.isFinite(Number(value));
  }
  if (t === "number") {
    // Same behavior for numbers: non-numeric values are considered empty first.
    // This keeps error messaging consistent with required checks.
    if (value === "") return true;
    return !Number.isFinite(Number(value));
  }
...
function validateFieldFormat(field, value) {
  ...
  if (t === "month") {
    if (typeof value !== "string" || !isValidMonthString(value)) {
      return `"${label}" must be a month in YYYY-MM format.`;
    }
    return null;
  }
  if (t === "select") {
...
export function validateCrudPayloadForWrite(moduleConfig, data, mode, keysInRequest) {
  ...
  if (mode === "create") {
    // CREATE: every required editable field must be present and well-formed.
