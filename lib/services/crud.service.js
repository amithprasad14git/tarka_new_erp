...
} from "../modules/newCaseInward";

/**
 * New Case Inward–only checks: `validateNewCaseInwardBeforeWrite` runs only when `moduleKey === "new_case_inward"`.
...
export async function updateCrudRecord(user, moduleKey, id, getRawBody) {
  ...
  if (moduleKey === "new_case_inward" && Number(user?.role) === 2) {
    const conn = await pool.getConnection();
    try {
      const locked = await isNewCaseInwardFinalStatusById(conn, oldRow?.caseStatus);
      if (locked) {
        return { status: 403, body: { error: "Final-stage cases cannot be edited." } };
      }
    } finally {
      conn.release();
    }
  ...
  if (moduleConfig.childTables?.length) {
    ...
    try {
      ...
      if (e?.code === "CHILD_ROWS_INVALID" || e?.code === "NCI_VALIDATION_FAILED") {
...
export async function createCrudRecord(user, moduleKey, rawBody) {
  ...
  // One transaction: insert + module-specific follow-up (see lib/moduleAfterCreate.js).
  const conn = await pool.getConnection();
  try {
    await conn.beginTransaction();
    if (moduleKey === "new_case_inward") {
    ...
    // Expected business/data errors from New Case Inward Case No logic (user-fixable).
    if (
