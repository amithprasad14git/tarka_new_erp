...
import { deleteChildTablesForParent, syncChildTablesInTransaction } from "../childTablesSync";
...
import { resolveCrudWriteErrorResponse } from "../crudSqlUserMessage";
...
export async function updateCrudRecord(user, moduleKey, id, getRawBody) {
  ...
  if (moduleConfig.childTables?.length || moduleAdapter?.requiresUpdateTransaction === true) {
    ...
    try {
      ...
      const writeErr = resolveCrudWriteErrorResponse(e);
  ...
  let body = moduleAdapter?.buildUpdateResponseBody
    ? moduleAdapter.buildUpdateResponseBody({ moduleConfig, id, savedRow, moduleKey })
    : { ok: true, id: Number(id) };

  if (!body.postCreateAck && moduleConfig?.postCreateAck?.showPrintPdf === true) {
    body = buildPostCreateAckUpdateBody(moduleConfig, id, savedRow);
  }
...
export async function createCrudRecord(user, moduleKey, rawBody) {
  ...
  try {
    ...
    if (ackCfg?.field) {
      const raw = rowValueForField(createdRow, ackCfg.field);
    ...
    return resolveCrudWriteErrorResponse(e);
  } finally {
    conn.release();
  }
}
 ...
 * row exists → row scope allows deleting this row → delete child rows (if configured)
 * → DELETE parent → audit log with old row snapshot and newData null (meaning “gone”).
 *
...
export async function deleteCrudRecord(user, moduleKey, id) {
  ...
  if (moduleConfig.childTables?.length) {
    ...
    try {
      await conn.beginTransaction();
      await deleteChildTablesForParent(conn, moduleConfig, Number(id));
      await conn.query(`DELETE FROM ${mt} WHERE id=?`, [id]);
      await conn.commit();
    } catch (error) {
      await conn.rollback();
      throw error;
